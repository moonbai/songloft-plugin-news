...
function initRouter() {
    ...
    router.post('/api/sources/import', async (req) => {
        try {
            ...
            const body = request.body;
            if (!body)
                return jsonResponse({ code: 400, msg: 'No body' }, 400);
            const text = new TextDecoder().decode(body);
            const parsed = JSON.parse(text);
            const name = String(parsed.name || 'imported');
            const content = String(parsed.content || '');
            if (!content)
                return jsonResponse({ code: 400, msg: 'content required' }, 400);
    ...
    router.post('/api/sources/import-url', async (req) => {
        try {
            ...
            if (!url)
                return jsonResponse({ code: 400, msg: 'url required' }, 400);
    ...
    router.delete('/api/sources', async (req) => {
        try {
            ...
            const id = query.id;
            if (!id)
                return jsonResponse({ code: 400, msg: 'id required' }, 400);
    ...
    router.post('/api/direct/music/url', async (req) => {
        try {
            ...
            const quality = String(parsed.quality || 'standard');
            if (!songInfo)
                return jsonResponse({ code: 400, msg: 'songInfo required' }, 400);
...
globalThis.onHTTPRequest = function (req) {
    try {
        ...
        songloft.log.debug(`HTTP request: ${method} ${path}`);
