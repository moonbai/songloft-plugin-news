...
export class RuntimeManager {
    constructor() {
        ...
        this.platformIndex = new Map(); // platform → runtimeIds[]
    }
    /** 加载音源 */
    async loadSource(id, name, script, scriptInfo) {
        // 如果已存在,先销毁
        ...
        const runtime = new SourceRuntime(id, name, script, scriptInfo);
    ...
    async getMusicUrl(songInfo, quality) {
        ...
        // 构造并行请求
        const promises = runtimes.map(rt => rt.getMusicUrl(songInfo, quality).then(result => ({ runtime: rt, result })));
        try {
            // 等第一个成功
            const firstSuccess = await Promise.race(promises.map(async (p) => {
