    ...
    if (Array.isArray(singer)) {
        return singer
            ...
            if (s && typeof s === 'object') {
                return s.name || s.singerName || s.artistName || '';
            }
            return String(s);
        })
            .filter(Boolean)
...
function buildSongInfo(raw) {
    const id = getField(raw, 'id', 'songId');
    const copyrightId = getField(raw, 'copyrightId', 'copyrightID', 'copyright');
    const idStr = id != null ? String(id) : '';
    const cpStr = copyrightId != null ? String(copyrightId) : '';
    return {
        platform: 'mg',
        name: getField(raw, 'songName', 'name', 'title') || '',
        singer: parseSinger(getField(raw, 'singer', 'singers', 'artist', 'artists')),
        album: getField(raw, 'albumName', 'album', 'albumTitle') || '',
        duration: parseDuration(getField(raw, 'duration', 'timeLength')),
        musicId: idStr,
        copyrightId: cpStr,
        songmid: cpStr || idStr,
        cover: getField(raw, 'cover', 'img', 'albumImg', 'pic', 'imageUrl', 'albumPic') ||
            '',
    };
}
...
async function getLyric(songInfo) {
    const copyrightId = String(songInfo.copyrightId || songInfo.musicId || songInfo.songmid || '');
    if (!copyrightId)
        return null;
    // 候选 endpoint,按序尝试
    const endpoints = [
    ...
    for (const url of endpoints) {
        ...
        // 多个可能的字段
        const lyricField = (body.lyric || body.content || body.result || body.lyrics || body.lrc);
        let text = '';
        if (typeof lyricField === 'string') {
            text = lyricField;
        }
        else if (lyricField && typeof lyricField === 'object') {
            text = lyricField.content || lyricField.lyric || '';
        }
        if (text && text.trim()) {
...
const songList = {
    async tags() {
        try {
            const data = await miguGet(MIGU_M_BASE + '/migu/remoting/cms_playlist_tag?msisdn=');
    ...
    async list(tag, page, limit) {
        ...
        const url = MIGU_M_BASE +
            ...
            limit;
        const data = await miguGet(url);
        const items = (data && (data.retItemList || data.playlistList || data.list)) || [];
    ...
    async detail(id, _page) {
        ...
        const data = await miguGet(url);
        const songs = (data && (data.songList || data.list)) || [];
        const list = songs.map(buildSongInfo);
        return {
            info: {
                id: String(id),
...
const leaderboard = {
    async boards() {
        try {
            const data = await miguGet(MIGU_M_BASE + '/migu/remoting/cms_topdata_list?msisdn=');
            const arr = Array.isArray(data)
                ? data
