...
function formatLrcLine(time, line) {
    ...
    async search(keyword, page, limit) {
        ...
        try {
            ...
            if (Array.isArray(abslist)) {
                songs = abslist.map((item) => mapSearchSong(item));
            }
            total = toNumber(body.HITSMATCHTOTAL ?? body.TOTAL ?? body.total ?? body.MUSIC_DANCE, songs.length);
        }
        catch (err) {
...
async function getLyric(songInfo) {
    ...
    if (!musicId)
        return null;
    const url = `http://m.kuwo.cn/newh5/singles/songinfoandlrc?musicId=${encodeURIComponent(String(musicId))}&httpsStatus=1`;
    try {
        const { promise } = httpFetch(url, {
            method: 'GET',
            headers: {
        ...
        const lyric = lrclist
            .map((l) => formatLrcLine(l.time, l.line))
...
function mapDetailSong(item) {
    const id = String(item.id || item.DC_TARGETID || item.musicrid || '');
    const duration = parseTimeToSeconds(item.songTimeMinutes) ||
        toNumber(item.duration, 0) ||
        toNumber(item.DURATION, 0);
    const cover = item.pic || item.albumpic || item.web_albumpic_short
...
const songList = {
    /** 获取歌单标签 */
    async tags() {
        const url = 'http://wapi.kuwo.cn/api/pc/classify/playlist/getTagList?cmd=rcm_keyword_playlist&user=0&prod=kwplayer_pc_9.0.5.0&vipver=12&level=8&auth=';
        try {
            const { promise } = httpFetch(url, {
                method: 'GET',
                headers: COMMON_HEADERS,
    ...
    async list(tag, page, limit) {
        const pn = Math.max((page || 1) - 1, 0);
        const rn = limit || 30;
        const url = `http://wapi.kuwo.cn/api/pc/classify/playlist/getTagPlayList?cmd=rcm_keyword_playlist&user=0&prod=kwplayer_pc_9.0.5.0&vipver=12&level=8&id=${encodeURIComponent(tag)}&pn=${pn}&rn=${rn}&`;
        let list = [];
        let total = 0;
        try {
            const { promise } = httpFetch(url, {
    ...
    async detail(id, page) {
        const pn = Math.max((page || 1) - 1, 0);
        const url = `http://nplserver.kuwo.cn/pl.svc?op=getlistinfo&pid=${encodeURIComponent(id)}&pn=${pn}&rn=100&encode=utf8&keyset=pl2012&identity=kuwo&pcmp4=1&vipver=MUSIC_9.0.5.0&newver=1`;
        let info = {
            id: String(id),
            name: '',
            cover: '',
        ...
        try {
            ...
            if (Array.isArray(musiclist)) {
                list = musiclist.map((item) => mapDetailSong(item));
            }
        }
        catch (err) {
...
const leaderboard = {
    ...
    async list(id, page, limit) {
        const pn = Math.max((page || 1) - 1, 0);
        const rn = limit || 30;
        const url = `http://kbangserver.kuwo.cn/ksong.svc?act=bang&bc=${encodeURIComponent(id)}&pn=${pn}&rn=${rn}&mobi=1`;
        let list = [];
        let total = 0;
        try {
            const { promise } = httpFetch(url, {
