...
function mapSong(raw) {
    ...
    return {
        ...
        singer: artists.map((a) => a.name).filter(Boolean).join(' / '),
        album: album.name || '',
...
async function fetchPlaylistTracks(id, page, limit) {
    const data = await getJSON(`${API_BASE}/api/playlist/detail?id=${encodeURIComponent(id)}`);
...
const musicSearch = {
    async search(keyword, page, limit) {
        ...
        const offset = (page - 1) * limit;
        const data = await postForm(`${API_BASE}/api/search/get/web`, {
            s: keyword,
            type: '1',
        ...
        const songs = (result.songs || []).map(mapSong);
        const total = result.songCount || songs.length;
...
async function getLyric(songInfo) {
    ...
    if (!musicId)
        return null;
    const data = await getJSON(`${API_BASE}/api/song/lyric?id=${encodeURIComponent(String(musicId))}&lv=1&kv=1&tv=-1`);
    if (!data)
        return null;
...
const songList = {
    /** 歌单分类标签 */
    async tags() {
        const data = await postForm(`${API_BASE}/api/playlist/catalogue`, {});
        if (!data)
            return { hot: [], tagsList: {} };
        const sub = data.sub || [];
        ...
        const result = { hot, tagsList: groupByCat };
    ...
    async list(tag, page, limit) {
        ...
        tag = tag || '全部';
        const offset = (page - 1) * limit;
        const data = await postForm(`${API_BASE}/api/playlist/list`, {
            cat: tag,
            offset: String(offset),
            limit: String(limit),
            order: 'hot',
...
const leaderboard = {
    ...
    async boards() {
        try {
            const data = await getJSON(`${API_BASE}/api/toplist`);
