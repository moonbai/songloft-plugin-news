...
import type { SourceMetadata, ZipEntry } from './types';

// ============ JSDoc 元数据解析 ============

/** 从脚本头 JSDoc 解析 @name/@version/@description/@author/@homepage */
export function parseScriptMetadata(script: string, fallbackName?: string): SourceMetadata {
...
export function latin1ToUtf8(latin1: string): string {
  try {
    // 先把每个字符转为 byte
 ...
 */
export function parseZip(latin1Data: string): ZipEntry[] {
  ...
  for (let i = 0; i < cdEntries; i++) {
    if (offset + 46 > latin1Data.length) break;
...
function readUint16(data: string, offset: number): number {
...
/** latin1 字符串转 hex */
function latin1ToHex(latin1: string): string {
  let hex = '';
  for (let i = 0; i < latin1.length; i++) {
...
export function parseMultipart(body: Uint8Array, contentType: string): {
  fields: Record<string, string>;
  files: Array<{ name: string; filename: string; content: string }>;
} {
  const fields: Record<string, string> = {};
  const files: Array<{ name: string; filename: string; content: string }> = [];
