...
function normalizeTrack(track: any): NewsItem {
  return {
...
const newsList = {
  async list(category: string, page: number, limit: number): Promise<NewsListResult> {
    ...
    const news = ((data?.data?.tracks as any[]) || []).map(normalizeTrack);
...
const newsSearch = {
  async search(keyword: string, page: number, limit: number): Promise<NewsSearchResult> {
    ...
    const data = resp.body as any;
    const tracks = ((data?.data?.tracks as any[]) || []);
    const news = tracks.map((t: any) => normalizeTrack(t.track || t));
